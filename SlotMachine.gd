...
func _setup_spin_purchase_ui_style():
	...
	spin_amount_box_style.border_width_bottom = 1
	...
	(spin_purchase_button.get_theme_stylebox("hover") as StyleBoxFlat).bg_color = Color(0.3, 0.6, 0.9, 1)
	spin_purchase_button.add_theme_stylebox_override("pressed", button_style.duplicate())
	(spin_purchase_button.get_theme_stylebox("pressed") as StyleBoxFlat).bg_color = Color(0.1, 0.4, 0.7, 1)
	spin_purchase_button.add_theme_stylebox_override("disabled", button_style.duplicate())
	(spin_purchase_button.get_theme_stylebox("disabled") as StyleBoxFlat).bg_color = Color(0.1, 0.1, 0.1, 0.5)
	(spin_purchase_button.get_theme_stylebox("disabled") as StyleBoxFlat).border_color = Color(0.1, 0.1, 0.1, 0.5)
...
func _create_icon(icon_name: String, is_double: bool = false) -> Control:
	...
	container.custom_minimum_size = Vector2(128, 128) # Match icon size
	...
	# No need to call _setup_icon on icon1, as its parent (container) handles sizing
	...
	if is_double:
		...
		# No need to call _setup_icon on icon2
		# Slightly offset the second icon
		...
		# The base icon name is already set on the container by icon1's meta
		# container.set_meta("icon_name", icon_name) # Redundant here
...
func _on_spin_amount_box_value_changed(value: float) -> void:
	...
	print("💰 Spin Purchase UI: Calculated cost for %d spins: %d coins (Global.spin_price_per_unit: %d)" % [value, final_cost, Global.spin_price_per_unit])
...
func spin():
	...
	if "hex_hand" in Global.bought_upgrades:
		...
		if best_reroll["reel_index"] != -1:
			...
			print("✨ Hex’s Hand triggered: rerolling reel %d for a better outcome. New icons: %s" % [reroll_index, new_icons])
...
func check_wins(_grid: Array) -> Dictionary: # Prefix grid with underscore
	...
	for y in range(3):
		for x in range(2):
			...
			if name1 != "" and name1 == name2 and name2 == name3:
				...
				total_payout += int(_get_icon_value_from_reel(x, y) + _get_icon_value_from_reel(x + 1, y) + _get_icon_value_from_reel(x + 2, y))
	...
	for y in range(3):
		...
		if name1 != "" and name1 == name2 and name2 == name3 and name3 == name4:
			...
			total_payout += int(_get_icon_value_from_reel(0, y) + _get_icon_value_from_reel(1, y) + _get_icon_value_from_reel(2, y) + _get_icon_value_from_reel(3, y))
	...
	for x in range(4):
		...
		if name1 != "" and name1 == name2 and name2 == name3:
			...
			total_payout += int(_get_icon_value_from_reel(x, 0) + _get_icon_value_from_reel(x, 1) + _get_icon_value_from_reel(x, 2))
	...
	for path in diag3:
		...
		if name1 != "" and name1 == name2 and name2 == name3:
			matches.append(path)
			total_payout += int(_get_icon_value_from_reel(path[0].x, path[0].y) + _get_icon_value_from_reel(path[1].x, path[1].y) + _get_icon_value_from_reel(path[2].x, path[2].y))
...
func _simulate_reroll_and_check_win(current_grid: Array, reel_index: int, new_icons: Array) -> Dictionary:
...
func _find_best_reroll(original_grid: Array, original_matches: Array) -> Dictionary:
	...
	var _current_payout = check_wins(original_grid)["payout"] # Prefix with underscore
	...
	for reel_idx in losing_reels:
		for _i in range(5): # Try multiple random sets of icons for the reel
			...
			var simulated_result = _simulate_reroll_and_check_win(original_grid, reel_idx, potential_new_icons)
...
func _inject_luck_symbols(_grid: Array):
	var luck_count = clamp(Global.luck, 0, 12)
	if luck_count == 0:
		return
	var chosen_icon_name = ICONS[randi() % ICONS.size()]
	var empty_positions = []
	for y in range(3):
		for x in range(reels.size()):
	...
	for i in range(min(luck_count, empty_positions.size())):
		...
		var new_icon = _create_icon(chosen_icon_name, true) # Create a double icon
